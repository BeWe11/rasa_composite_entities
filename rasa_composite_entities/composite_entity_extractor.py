...
class CompositeEntityExtractor(EntityExtractor):
    ...
    Example:
        "composite_entities": [
          {
            "name": "name1",
            "patterns": ["@location @cuisine", "@location and @cuisine"]
          },
          {
            "name": "name2",
            "patterns": ["@cuisine @location"]
          }
        ],
        "common_examples": [
          {
    ...
    If a pattern matches, the contained entities will be removed from the
    normal entity list in the message object and instead be put into a new
    composite entity entry. This component introduces a new key for entity
    objects: the "type" key can either be set to "composite" or "basic", so
    that entity types can be recognized when parsing the output messages.
